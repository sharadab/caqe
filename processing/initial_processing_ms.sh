...
# It registers CALIPR and TVDE images to then do clustering.
# This version is for MS only, HCs use the initial_processing_hc script.
# This is because CSF masking in MS is a little trickier.
...
############# Now deal with registration. At the end of this, we want MWF, uFA and CMD maps all registered together. ######
...
##Register CALIPR to T1w, affine reg. Also move over MWF for atlas building later on.
...
###### It is difficult to CSF-mask MS brains accurately! We do it as a 2-step process. ##########
##### We need to make a CSF and tissue mask from both FLAIR and T1w (since using just T1w doesn't work with periventricular lesions). Then we combine them.####
## First put the FLAIR in an ANTs folder, brain extract and warp it to CALIPR.
mkdir ${folder_ants}/FLAIR
echo "Brain extracting FLAIR"
cp ${folder_nifti}/*FLAIR*.nii.gz ${folder_ants}/FLAIR/FLAIR.nii.gz
fslreorient2std ${folder_ants}/FLAIR/FLAIR.nii.gz ${folder_ants}/FLAIR/FLAIR.nii.gz
antsBrainExtraction.sh -d 3 -a ${folder_ants}/FLAIR/FLAIR.nii.gz -e ../NKI_Template/T_template.nii.gz -m ../NKI_Template/T_template_BrainCerebellumProbabilityMask.nii.gz -o ${folder_ants}/FLAIR/
cp ${folder_ants}/FLAIR/BrainExtractionBrain.nii.gz ${folder_ants}/FLAIR/FLAIR_brain.nii.gz 


##Warp to CALIPR space. Affine transform.
echo "Warping FLAIR to CALIPR space"
mkdir ${folder_ants}/CALIPR_FLAIR
antsRegistrationSyNQuick.sh -d 3 -f ${folder_ants}/CALIPR/${subject}_E1.nii.gz -m ${folder_ants}/FLAIR/FLAIR.nii.gz -o ${folder_ants}/CALIPR_FLAIR/ -n 8 -t a


##Make a CSF and no-csf mask from T1w images, warp to CALIPR space. This seems to exclude some periventricular lesions.
echo "Making CSF and no-CSF masks from T1w"
...
antsApplyTransforms -d 3 -e 0 -i ${folder_ants}/3DT1/no_csf_mask.nii.gz -r ${folder_ants}/CALIPR/${subject}_E1.nii.gz -t ${folder_ants}/CALIPR_3DT1/0GenericAffine.mat -o ${folder_ants}/CALIPR/t1_no_csf_mask.nii.gz
fslmaths ${folder_ants}/CALIPR/t1_no_csf_mask.nii.gz -thr 0.5 -bin ${folder_ants}/CALIPR/t1_no_csf_mask.nii.gz


##Do the same with FLAIR, make a CSF and no-CSF mask and warp to CALIPR space. This seems to exclude some iron-rich brain parts.
echo "Making CSF and no-CSF masks from FLAIR"
Atropos -d 3 -a ${folder_ants}/FLAIR/FLAIR_brain.nii.gz -i Otsu[3] -x ${folder_ants}/FLAIR/BrainExtractionMask.nii.gz -o ${folder_ants}/FLAIR/segmented.nii.gz
fslmaths ${folder_ants}/FLAIR/segmented.nii.gz -thr 2 -uthr 3 -bin ${folder_ants}/FLAIR/no_csf_mask.nii.gz
antsApplyTransforms -d 3 -e 0 -i ${folder_ants}/FLAIR/no_csf_mask.nii.gz -r ${folder_ants}/CALIPR/${subject}_E1.nii.gz -t ${folder_ants}/CALIPR_FLAIR/0GenericAffine.mat -o ${folder_ants}/CALIPR/flair_no_csf_mask.nii.gz
fslmaths ${folder_ants}/CALIPR/flair_no_csf_mask.nii.gz -thr 0.5 -bin ${folder_ants}/CALIPR/flair_no_csf_mask.nii.gz
antsApplyTransforms -d 3 -e 0 -i ${folder_ants}/FLAIR/FLAIR_brain.nii.gz -r ${folder_ants}/CALIPR/${subject}_E1.nii.gz -t ${folder_ants}/CALIPR_FLAIR/0GenericAffine.mat -o ${folder_ants}/CALIPR_FLAIR/FLAIR_brain.nii.gz


##Add the two masks (from FLAIR and T1w) to create an overall no-CSF mask that includes lesions.
fslmaths ${folder_ants}/CALIPR/flair_no_csf_mask.nii.gz -add ${folder_ants}/CALIPR/t1_no_csf_mask.nii.gz -bin ${folder_ants}/CALIPR/mask_no_csf.nii.gz
##Make a CSF mask by removing the tissue mask from the overall brain mask.
fslmaths ${folder_ants}/CALIPR/BrainExtractionMask_ero.nii.gz -sub ${folder_ants}/CALIPR/mask_no_csf.nii.gz -bin ${folder_ants}/CALIPR/csf_mask.nii.gz
